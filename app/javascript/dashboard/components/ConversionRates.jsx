...
import Style from './ConversionRates.scss'
...
class ConversionRates extends React.Component {
  constructor(props) {
    super(props)
    ...
    this.rateParameter = this.props.rateParameter
  }
  ...
  leadSearchLink = (parameter, value) => {
    let property_filter = ""
    let user_filter = ""
    ...
    if (this.props.filters != undefined && this.props.filters.users.length > 0 && this.rateParameter != 'user_ids') {
      for (var p of this.props.filters.users) {
        user_filter = user_filter + `&lead_search[user_ids][]=${p.val}`
      }
    }
    return(`/leads/search?lead_search[${parameter}][]=${value}${property_filter}${user_filter}`)
  }
  ...
  handleBarMouseUp = (d) => {
    this.openLinkInTab(this.leadSearchLink(this.rateParameter,d.id))
  }
  ...
  addAxesLabels = () => {
    ...
    chart.select("text.axis--x-label")
      .attr("transform", `translate(${this.props.width / 2}, ${this.props.height - 10})`)
  ...
  render(){
    return(
      <div className={Style.ConversionRates}>
...
ConversionRates.propTypes = {
  ...
  rateParameter: PropTypes.string.isRequired
}

ConversionRates.defaultProps = {
  filters: {},
  selectX: () => {},
  selectX: () => {},
  height: 300,

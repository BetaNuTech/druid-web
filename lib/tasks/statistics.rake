namespace :statistics do
  ...
  task :bootstrap => :environment do
    if ContactEvent.count < 100
      ...
      Lead.where(created_at: 2.months.ago..Time.now).each do |lead|
    ...
    Statistic.generate_leadspeed(resolution: 60, time_start: 2.months.ago.beginning_of_month)

    puts "*** Generating Property Leadspeed for the past 3 months"
    Property.active.each{|property| Statistic.generate_property_leadspeed(property: property, resolution: 60, time_start: 2.months.ago.beginning_of_month)}

    puts "*** Generating Property Leadspeed for the past 2 months"
    Team.all.each{|team| Statistic.generate_team_leadspeed(team: team, resolution: 60, time_start: 2.months.ago.beginning_of_month)}

    puts "*** Rolling up Leadspeed by day for the past 2 months"
    60.times{|i| Statistic.rollup_leadspeed(interval: :day, time_start: (i+1).days.ago.beginning_of_day)}

    puts "*** Rolling up Leadspeed by week for the past 2 months"
    8.times{|i| Statistic.rollup_leadspeed(interval: :week, time_start: (i+1).weeks.ago.beginning_of_week)}

    puts "*** Rolling up Leadspeed by week for the past 2 months"
    2.times{|i| Statistic.rollup_leadspeed(interval: :month, time_start: (i+1).months.ago.beginning_of_month)}
  end

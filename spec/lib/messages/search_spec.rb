...
RSpec.describe Messages::Search do
  ...
  describe "#base_scope" do
    let(:search) { Messages::Search.new(params: {}) }
    ...
    context "when user is an agent" do
      ...
      subject { search.send(:base_scope, user: agent1) }
      ...
      context "when a lead is reassigned" do
        it "message visibility follows the new assignment" do
          ...
          initial_scope = search.send(:base_scope, user: agent1)
          expect(initial_scope).to include(system_message_to_agent1_lead)
          ...
          new_scope = search.send(:base_scope, user: agent1)
          expect(new_scope).not_to include(system_message_to_agent1_lead)

          # And agent1_alt should see it
          agent1_alt_scope = search.send(:base_scope, user: agent1_alt)
    ...
    context "when user is an administrator" do
      ...
      subject { search.send(:base_scope, user: administrator) }
    ...
    context "when user is a manager" do
      ...
      subject { search.send(:base_scope, user: manager_user) }
    ...
    context "when current_property is specified" do
      let(:manager_user) do
        user = create(:user, role: manager_role)
        user.confirm
        property1.assign_user(user: user, role: 'manager')
      ...
      let!(:message_at_property1) do
        create(:message,
          user: agent1,
          messageable: lead_assigned_to_agent1,
          message_type: email_message_type,
          body: "Message at property1",
          subject: "Test",
          incoming: false)
      end

      let!(:message_at_property2) do
        create(:message,
          user: agent2,
          messageable: lead_assigned_to_agent2,
          message_type: email_message_type,
          body: "Message at property2",
          subject: "Test",
          incoming: false)
      end

      subject { search.send(:base_scope, user: manager_user, current_property: property1) }

      it "includes only messages for the specified property" do
        expect(subject).to include(message_at_property1)
        expect(subject).not_to include(message_at_property2)
      end
    end
  end
end
